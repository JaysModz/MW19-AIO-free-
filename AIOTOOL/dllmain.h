...
LRESULT hkWndProc(HWND hWnd, UINT Msg, WPARAM wp, LPARAM lp)
{
	...
	io.WantCaptureMouse = JaysModz->msettings.b_menuEnable ? true : false;
...
void WndProc_hk()
{
	ogr_function::WndProc = (WNDPROC)SetWindowLongPtrW(JaysModz->hWind, GWLP_WNDPROC, (LONG_PTR)hkWndProc);
}
...
namespace imgui
{
	...
	ImFont* start(IDXGISwapChain3* pSwapChain, ID3D12CommandQueue* pCommandQueue, type::tImguiStyle SetStyleFunction, float font_size)
	{
		...
		ImGui_ImplWin32_Init(JaysModz->hWind);
		ImGui_ImplDX12_Init(
			d3d12::pDevice,
		...
		//MainCaps = ImGui::GetIO().Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\fixedsysregular.ttf", 20);
		//Main = ImGui::GetIO().Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\fixedsysregular.ttf", 10);
		//FMenu = ImGui::GetIO().Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\FranklingothicMedium.ttf", 50);
		...
		headerFont = ImGui::GetIO().Fonts->AddFontFromMemoryCompressedTTF(Roboto_compressed_data, Roboto_compressed_size, 26.f, nullptr, ImGui::GetIO().Fonts->GetGlyphRangesCyrillic());
		bodyFont = ImGui::GetIO().Fonts->AddFontFromMemoryCompressedTTF(Roboto_compressed_data, Roboto_compressed_size, 22.f, nullptr, ImGui::GetIO().Fonts->GetGlyphRangesCyrillic());
		bodyFont2 = ImGui::GetIO().Fonts->AddFontFromMemoryCompressedTTF(Roboto_compressed_data, Roboto_compressed_size, 20.f, nullptr, ImGui::GetIO().Fonts->GetGlyphRangesCyrillic());
